...
[plt.plot(x_regressor, y_nn_, label=f'Epochs = {50*((i*4)+1)}') for y_nn_, i in zip(y_nn[::4], range(int(number_trainers/4)))]
plt.legend()
plt.grid()
plt.xlabel('Time')
plt.ylabel('Amplitude')
plt.savefig('data/NeuralNetwork_Epochs.png')
plt.show()

error_NeuralNet = [sk_mse(y_regressor, y_nn_) for y_nn_ in y_nn]
plt.figure()
plt.title('Error over Epochs - Neural Net')
plt.semilogy(np.linspace(50, number_trainers*50, number_trainers), error_NeuralNet)
plt.legend()
plt.grid()
plt.xlabel('Epochs')
plt.ylabel('Error (mse)')
plt.savefig('data/NeuralNetwork_Error.png')
plt.show()

# Assume to not know the truth anymore
plt.figure()
plt.title('Neural Network loss function over Epochs \n We dont know the truth anymore')
plt.semilogy(np.linspace(50, number_trainers*50, number_trainers), loss_mse)
plt.grid()
plt.xlabel('Epochs')
plt.ylabel('MSE')
plt.savefig('data/NeuralNetwork_Loss.png')
plt.show()
...
plt.xlabel('Time')
plt.ylabel('Amplitude')
plt.savefig('data/PINNs_Epochs.png')
plt.show()

y_PINN_true = exact_solution(d, w0, t_test)
error_PINN = [sk_mse(y_PINN_true, y_nn_) for y_nn_ in pinn_solution]
plt.figure()
plt.title('Error over Epochs - PINN')
plt.semilogy(np.linspace(0, 4_000*3, 4), error_PINN)
plt.grid()
plt.xlabel('Epochs')
plt.ylabel('Error (mse)')
plt.savefig('data/PINNs_Error.png')
plt.show()

# Assume to not know the truth anymore
plt.figure()
plt.title('PINNs loss function over Epochs \n We dont know the truth anymore')
plt.semilogy(np.linspace(0, 4_000*3, 4), loss_PINN)
plt.grid()
plt.xlabel('x')
plt.ylabel('MSE')
plt.savefig('data/PINNs_Loss.png')
plt.show()
